...
<script>
import Api from './api'
import SelectPositionModal from './components/SelectPositionModal'
import BimShow from '../home/components/bimShow'
export default {
  name: 'QualityQuestionAdd',
  components: {
    SelectPositionModal,
    BimShow
  },
  data() {
    return {
      ...
      formRules: {
        ...
        engineeringCategory: [{ required: true, message: '请选择工程类型', trigger: 'change' }],
  ...
  watch: {
    'form.engineeringCategory'(val) {
      this.form.issueType = ''
      this.$nextTick(() => {
        this.$refs.formRef.clearValidate('issueType')
      })
    }
  },
  // 生命周期 - 挂载完成（可以访问DOM元素）
  ...
  methods: {
    ...
    changeAnchor({ componentIds, componentPos }) {
      console.log('获取添加的点位构件ID集合和坐标集合', componentIds, componentPos)
