  ...
  <div class="container">
    <div class="tooltips">
      <el-form ref="form" inline :model="filterForm">
        ...
        <br />
        <el-form-item>
          <el-button v-if="$hasPower('InvestmentBudgetAddEpcPpp')" type="primary" icon="el-icon-plus" @click="add"
            >新增</el-button
          >
        </el-form-item>
      </el-form>
    ...
    <Grid ref="table" :request="request" :filter-form.sync="filterForm">
      ...
      <vxe-table-column field="name" title="施工图预算名称" width="200"></vxe-table-column>
      <vxe-table-column field="money" title="预算总额(万元)" width="200"></vxe-table-column>
      <vxe-table-column field="preName" title="投资概算名称" width="200">
        <template #default="{ row }">
          <a @click="toEstimate(row.lastBudgetId)">{{ row.preName }}</a>
        </template>
      </vxe-table-column>
      <vxe-table-column field="preMoney" title="概算总额(万元)" width="200"></vxe-table-column>
      ...
      <vxe-table-column title="操作" width="150" fixed="right">
        <template #default="{ row }">
          <el-button v-if="$hasPower('InvestmentBudgetEditEpcPpp')" type="text" @click="toEdit(row)">编辑</el-button>
          <el-button v-if="$hasPower('InvestmentBudgetDeleteEpcPpp')" type="text" @click="handleDelete(row)"
            >删除</el-button
          >
          <el-button v-if="$hasPower('InvestmentBudgetDetailEpcPpp')" type="text" @click="toDetail(row)"
            >查看</el-button
          >
        </template>
      </vxe-table-column>
...
export default {
  ...
  methods: {
    ...
    //添加按钮
    add() {
      this.$router.push({ name: 'InvestmentBudgetAddEpcPpp' })
    },
    //删除操作
