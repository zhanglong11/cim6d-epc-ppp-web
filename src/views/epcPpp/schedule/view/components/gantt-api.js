  ...
  function add_export_methods(gantt) {
    ...
    gantt._serialize_scales = function (config) {
      ...
      for (var i = 0; i < raw.length; i++) {
        ...
        var template =
          step.format || step.template || (step.date ? gantt.date.date_to_str(step.date) : gantt.config.date_scale)
    ...
    if (config && config.project) {
      for (var i in config.project) {
        ...
        gantt.config.$custom_data[i] =
          typeof config.project[i] === 'function' ? config.project[i](gantt.config) : config.project[i]
      }
      delete config.project
    }
  }
    ...
    if (config && config.tasks) {
      data.data.forEach(function (el) {
        for (var i in config.tasks) {
          ...
          el.$custom_data[i] =
            typeof config.tasks[i] === 'function' ? config.tasks[i](el, gantt.config) : config.tasks[i]
        }
      })
      delete config.tasks
    }
