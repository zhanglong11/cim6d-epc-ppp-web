    ...
    <div class="innerContainer white">
      <div class="formWrapper">
        <el-form ref="formRef" :model="form" :rules="formRules" label-width="140px" class="form-con">
          <el-form-item label="施工图预算名称" prop="name">
            <el-input v-model="form.name" placeholder="请输入预算名称" class="inputStyle"> </el-input>
          </el-form-item>
          <el-form-item label="预算金额(万元)" prop="money">
            <InputNumber v-model="form.money" placeholder="请输入预算金额" class="inputStyle"> </InputNumber>
          </el-form-item>
          <el-form-item label="投资概算名称" prop="lastBudgetId">
            <el-select v-model="form.lastBudgetId" clearable placeholder="请选择概算名称" class="inputStyle">
              <el-option v-for="item in estimateList" :key="item.id" :label="item.name" :value="item.id"> </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="概算金额(万元)" prop="preMoney">
...
export default {
  ...
  data() {
    return {
      ...
      estimateList: [],
      formRules: {
        name: [{ required: true, message: '施工图预算名称必填', trigger: 'blur' }],
        money: [{ required: true, message: '施工图预算金额必填', trigger: 'blur' }],
        lastBudgetId: [{ required: true, message: '投资概算必选', trigger: 'change' }]
      }
    }
  },
  computed: {},
  watch: {
    'form.lastBudgetId'(val) {
      let selectData = this.estimateList.filter(item => item.id === val)
  ...
  methods: {
    ...
    async getEstimateList() {
      let res = await Api.getEstimateList({
      ...
      if (this.id) {
        ...
        this.estimateList = res.data.records.filter(item => item.usedFlag === 0) || []
      }
    },
    //获取详情
    async getDetail() {
      let res = await Api.getBudgetDetail(this.id)
      this.form = { ...res.data }
    },
    //点击保存
    async handleSave() {
