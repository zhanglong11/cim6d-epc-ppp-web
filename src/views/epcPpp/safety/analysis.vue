      ...
      <div v-if="selectTreeNode" class="right">
        <div class="chartContainer">
          ...
          <div v-if="[1, 2].includes(selectTreeNode.type)" class="rightCharts">
            ...
            <p class="alignCenter">{{ selectTreeNode.label }}不同种类的安全文明问题数量占比统计</p>
          </div>
        </div>
        <div class="tableContainer">
          <template v-if="selectTreeNode.type === 0">
            ...
            <p class="alignCenter">安全文明问题数据对比分析表</p>
          </template>
          <template v-if="selectTreeNode.type === 1">
            <div class="table">
            ...
            <p class="alignCenter">{{ selectTreeNode.label }}安全文明问题数据对比分析表</p>
          </template>
          <template v-if="selectTreeNode.type === 2">
            <div class="table">
...
export default {
  ...
  data() {
    return {
      ...
      filterForm: { beginDate: null, endDate: null, projectId: localStorage.getItem('projectId') },
      request(body) {
  ...
  methods: {
    ...
    async getIssueTypeByYear() {
      ...
      let labelArr =
        res.data && res.data.map(item => this.$getLabel(this.safetyQuestionTypeList, item.engineeringCategory))
      let valueArr =
        res.data &&
        res.data.map(item => {
          return {
