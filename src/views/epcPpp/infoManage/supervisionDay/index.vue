<template>
  <div>
    <el-form ref="pageQuery" :model="filterForm" inline>
      <el-form-item label="">
        <el-input v-model="filterForm.name" placeholder="创建人/日报名称" style="width: 180px">
      ...
      <br />
      <el-form-item>
        <el-button v-if="$hasPower('SupervisionDayAddEpcPpp')" type="primary" icon="el-icon-plus" @click="toEdit(null)"
          >新建</el-button
        >
      </el-form-item>
    </el-form>
    <Grid ref="table" :request="request" :filter-form.sync="filterForm">
      ...
      <vxe-table-column title="日期" field="reportDate"> </vxe-table-column>
      ...
      <vxe-table-column title="操作">
        <template #default="{ row }">
          ...
          <el-button
            v-if="row.status === 0 && $hasPower('SupervisionDayEditEpcPpp')"
            type="text"
            @click="toEdit(row.id)"
            >编辑</el-button
          >
          <el-button
            v-if="row.status === 0 && $hasPower('SupervisionDayDeleteEpcPpp')"
            type="text"
            @click="remove(row.id)"
            >删除</el-button
          >
